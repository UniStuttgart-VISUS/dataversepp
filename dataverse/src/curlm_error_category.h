...
namespace detail {
    ...
    /// <summary>
    /// An error category for error codes of the cURL multi interface.
    /// </summary>
    class curlm_error_category final : public std::error_category {
    ...
    public:
        ...
        inline _Ret_z_ const char *name(void) const noexcept override {
            return "cURL multi interface";
        }
    };


    /// <summary>
    /// Answer the one and only <see cref="curlm_error_category" />.
    /// </summary>
    /// <returns>The only instance of <see cref="curlm_error_category" />.
    /// </returns>
    _Ret_valid_ const std::error_category& curlm_category(void) noexcept;
...
namespace std {
    ...
    inline std::error_code make_error_code(
            _In_ const CURLMcode e) noexcept {
